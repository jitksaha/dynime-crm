# Horilla CRM — Docker Deployment Guide

A complete, step-by-step guide to running Horilla CRM using Docker. Covers development setup, production deployment, customization, troubleshooting, and maintenance.
...
Get Horilla CRM running in under 5 minutes:

```bash
# 1. Clone the repository
git clone https://github.com/your-org/horilla-crm.git
cd horilla-crm
...
|---------|-------|---------|
| **web** | Custom (Dockerfile) | Django application with Uvicorn ASGI server (HTTP + WebSocket) |
| **celery-worker** | Custom (Dockerfile) | Background tasks: automations, email sending, scheduled exports |
| **celery-beat** | Custom (Dockerfile) | Periodic task scheduler: fiscal year updates, recyclebin cleanup |
| **db** | `postgres:16-alpine` | Primary database for all application data |
| **redis** | `redis:7-alpine` | Celery broker/result backend, Channels layer, caching (password-protected, AOF persistence) |
| **nginx** | `nginx:alpine` | Reverse proxy, static file serving, WebSocket proxy (production only) |
...
| `CELERY_BROKER_URL` | `redis://:...@redis:6379/0` | Celery message broker (uses Redis) |
| `CELERY_RESULT_BACKEND` | `redis://:...@redis:6379/0` | Celery result storage (uses Redis) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `UVICORN_WORKERS` | Auto (2–8) | Override worker count |
| `UVICORN_LOG_LEVEL` | `info` | Logging verbosity: debug, info, warning, error |
| `UVICORN_RELOAD` | `false` | Set to `true` for auto-reload during development |
...
The development setup mounts your local code into the container (`.:/app`), so code changes are reflected immediately. To enable Uvicorn auto-reload:

```bash
# Option 1: Set environment variable
docker compose exec web env UVICORN_RELOAD=true python docker/uvicorn.conf.py
...
- `web` service: `DATABASE_URL`, `REDIS_URL`, `CELERY_BROKER_URL`, `CELERY_RESULT_BACKEND`
- `celery-worker` and `celery-beat` services: same Redis/DB URLs
...
- Proxies HTTP requests to Django
- Proxies WebSocket connections (`/ws/`) with Upgrade headers
...
From the Horilla login page, click "Load Demo Data" to populate the system with sample leads, accounts, contacts, and other CRM data.
...
User uploads (profile photos, mail attachments, documents) are stored in the `media` volume.
...
Add cloud storage credentials to your environment:
...
The default configuration (`docker/uvicorn.conf.py`) auto-scales workers based on CPU count:

| Setting | Default | Description |
|---------|---------|-------------|
| Workers | `max(2, min(CPU*2+1, 8))` | Process count |
| WebSocket ping | `20s` interval / timeout | Keeps WebSocket connections alive |
| Lifespan | `on` | Enables ASGI lifespan protocol |
...
The Celery worker runs with 2 concurrent workers by default. Adjust in `docker-compose.yml`:
...
```bash
# Check web logs for Channels errors
make logs-web

# Verify Redis is healthy
docker compose exec redis redis-cli -a horilla_pass PING
```

If running behind Nginx, ensure the `/ws/` proxy configuration is present in `docker/nginx.conf`.
...
1. Check worker count: `docker compose exec web ps aux | grep uvicorn`
2. Check Celery queue: `docker compose exec celery-worker celery -A horilla inspect active`
3. Check database connections: `docker compose exec db psql -U horilla_user -c "SELECT count(*) FROM pg_stat_activity;"`
4. Check Redis memory: `docker compose exec redis redis-cli -a horilla_pass INFO memory`
...
| Command | Description |
|---------|-------------|
| `make dev` | Start all services (web + db + redis + celery) |
...
| `make worker` | Start Celery worker standalone |
| `make beat` | Start Celery beat standalone |
...
docker-compose.yml       # Service orchestration
                           - web: Django + Uvicorn ASGI (healthchecked)
